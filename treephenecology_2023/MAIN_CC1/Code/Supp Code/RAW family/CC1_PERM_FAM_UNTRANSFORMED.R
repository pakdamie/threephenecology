...
#####################################################################
#####################################################################
family_count_CC1<- count(species_name_CC1_Found, family)
...
for (k in seq(1,19)){
  ...
  assign(paste("PERM",13, sep = ""), tmp)
  
  saveRDS(PERM13,
       file =paste("PERM",13,".RDS", sep = ""))
  rm(list=paste("PERM",13, sep = ""))
  ...
  for (r in seq(5,nrow(family_five_CC1))){
    ...
    WMR_CC1_PERM_DF[[r]]<-
      Permutation_Actual_Combiner(MVCWT_CC1_WMR_COI0,PERM,WMR_FAM )
...
for (f in seq(1, nrow( family_five_CC1 ))){
  ...
  ANN_CC1_PERM $Cut <- cut(ANN_CC1_PERM $scale , breaks = c(seq(1,2.1,0.2)))
